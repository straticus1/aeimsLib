...
program
    ...
    .action(async (options) => {
    const spinner = (0, ora_1.default)('Verifying configuration...').start();
    try {
        const result = await config.verify(!options.skipTests);
        spinner.stop();
        if (result.success) {
            console.log(chalk_1.default.green('✓ Configuration verified successfully'));
        }
        else {
            console.log(chalk_1.default.red('✗ Configuration verification failed:'));
...
program
    ...
    .action(async (options) => {
    const spinner = (0, ora_1.default)('Setting up configuration...').start();
    try {
        const result = await config.setup(options);
        spinner.stop();
        if (result.success) {
            console.log(chalk_1.default.green('✓ Configuration setup completed'));
            console.log('\nConfiguration saved to:', result.configPath);
        }
        else {
            console.log(chalk_1.default.red('✗ Configuration setup failed:'));
            console.log(chalk_1.default.yellow(`  ${result.error}`));
            process.exit(1);
        }
    }
    catch (error) {
...
program
    ...
    try {
        if (options.interactive) {
            ...
            if (result.success) {
                console.log(chalk_1.default.green(`✓ Setting '${options.setting}' updated`));
            }
            else {
                console.log(chalk_1.default.red(`✗ Failed to update setting: ${result.error}`));
                process.exit(1);
            }
        }
        else {
            console.log(chalk_1.default.yellow('Please provide --setting and --value or use --interactive'));
            process.exit(1);
        }
    }
    catch (error) {
...
program
    ...
    try {
        ...
        results.forEach(result => {
            if (result.passed) {
                console.log(chalk_1.default.green(`✓ ${result.name}`));
            }
            else {
                console.log(chalk_1.default.red(`✗ ${result.name}`));
                console.log(chalk_1.default.yellow(`  ${result.error}`));
            }
            if (options.verbose && result.details) {
                console.log(chalk_1.default.gray(JSON.stringify(result.details, null, 2)));
            }
        });
...
program
    ...
    try {
        ...
        if (result.success) {
            console.log(chalk_1.default.green(`✓ Setting '${setting}' removed`));
        }
        else {
            console.log(chalk_1.default.red(`✗ Failed to remove setting: ${result.error}`));
            process.exit(1);
        }
    }
    catch (error) {
...
program
    ...
    const spinner = (0, ora_1.default)('Installing configuration...').start();
    try {
        const result = await config.install(source, options);
        spinner.stop();
        if (result.success) {
            console.log(chalk_1.default.green('✓ Configuration installed successfully'));
...
program
    ...
    try {
        ...
        const spinner = (0, ora_1.default)('Uninstalling configuration...').start();
        const result = await config.uninstall();
        spinner.stop();
        if (result.success) {
            console.log(chalk_1.default.green('✓ Configuration uninstalled successfully'));
        }
        else {
            console.log(chalk_1.default.red('✗ Uninstallation failed:'));
            console.log(chalk_1.default.yellow(`  ${result.error}`));
            process.exit(1);
        }
    }
...
program.parse();
//# sourceMappingURL=aeims-config.js.map