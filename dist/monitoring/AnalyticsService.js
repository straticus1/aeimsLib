...
class AnalyticsService extends events_1.EventEmitter {
    ...
    trackDeviceMetrics(deviceId, event) {
    ...
    // Session tracking
    startSession(sessionId, userId, deviceId) {
        const session = {
            sessionId,
            userId,
            deviceId,
    ...
    getPerformanceMetrics(timeRange) {
        ...
        return this.performanceLog.filter(metrics => metrics.timestamp >= timeRange.start && metrics.timestamp <= timeRange.end);
    }
    // Utility methods
    createDefaultMetrics(deviceId) {
        return {
            deviceId,
    ...
    // Clean up old data
    cleanupOldData(retentionPeriod = 30 * 24 * 60 * 60 * 1000) {
