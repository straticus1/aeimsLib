...
function activate(extensionContext) {
    ...
    // Register Commands
    const commands = [
        vscode.commands.registerCommand('aeimslib.openPatternDesigner', openPatternDesigner),
        vscode.commands.registerCommand('aeimslib.startSimulation', startDeviceSimulation),
        vscode.commands.registerCommand('aeimslib.startAnalyzer', startProtocolAnalyzer),
        vscode.commands.registerCommand('aeimslib.insertPattern', insertPatternTemplate),
        vscode.commands.registerCommand('aeimslib.validatePattern', validateCurrentPattern)
    ];
...
async function openPatternDesigner() {
    ...
    const panel = vscode.window.createWebviewPanel('aeimsPatternDesigner', 'Pattern Designer', vscode.ViewColumn.One, {
        enableScripts: true
    });
    // Initialize designer
    context.patternDesigner = new PatternDesigner_1.PatternDesigner();
    // Set webview HTML
    ...
    panel.webview.onDidReceiveMessage(async (message) => {
        switch (message.command) {
            case 'createPattern':
                try {
                    const pattern = context.patternDesigner.createStep(message.type, message.params);
                    panel.webview.postMessage({ type: 'patternCreated', pattern });
                }
                catch (error) {
                    vscode.window.showErrorMessage(`Failed to create pattern: ${error.message}`);
                }
                break;
...
async function startDeviceSimulation() {
    const deviceType = await vscode.window.showQuickPick(['vibrator', 'stroker', 'estim', 'tens'], { placeHolder: 'Select device type to simulate' });
    if (!deviceType)
        return;
    try {
        context.simulator = new DeviceSimulator_1.DeviceSimulator({
            id: `sim_${Date.now()}`,
        ...
        context.simulator.on('stateChanged', state => {
            ...
            output.appendLine(`Success Rate: ${(state.metrics.commandsSucceeded / state.metrics.commandsReceived * 100)
                .toFixed(1)}%`);
        });
        // Add status bar item
        ...
        vscode.window.showErrorMessage(`Failed to start simulation: ${error.message}`);
    }
}
/**
...
async function startProtocolAnalyzer() {
    context.analyzer = new ProtocolAnalyzer_1.ProtocolAnalyzer();
    // Create output channel
    ...
    context.analyzer.on('message', message => {
        output.appendLine(`[${new Date().toISOString()}] ${message.type} - ${message.raw.length} bytes`);
    });
    // Handle analysis
    context.analyzer.on('analysis', analysis => {
        ...
        output.appendLine('\nTiming Analysis:');
        output.appendLine(`Avg Interval: ${analysis.timingAnalysis.averageInterval.toFixed(2)}ms`);
        output.appendLine(`Burst Detected: ${analysis.timingAnalysis.burstDetected ? 'Yes' : 'No'}`);
    });
    // Add status bar item
    const statusBar = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left);
...
async function insertPatternTemplate() {
    ...
    const deviceTypes = Object.keys(patterns_1.DevicePatterns);
    const deviceType = await vscode.window.showQuickPick(deviceTypes, { placeHolder: 'Select device type' });
    if (!deviceType)
        return;
    const patterns = Object.keys(patterns_1.DevicePatterns[deviceType]);
    const patternName = await vscode.window.showQuickPick(patterns, { placeHolder: 'Select pattern' });
    if (!patternName)
        return;
    const pattern = patterns_1.DevicePatterns[deviceType][patternName];
...
async function validateCurrentPattern() {
    ...
    try {
        ...
        if (validation.valid) {
            ...
            vscode.window.showErrorMessage(`Pattern validation failed:\n${details}`);
        }
        if (validation.warnings.length > 0) {
            const details = validation.warnings.join('\n');
            vscode.window.showWarningMessage(`Pattern warnings:\n${details}`);
        }
    }
    catch (error) {
        vscode.window.showErrorMessage(`Failed to validate pattern: ${error.message}`);
    }
}
/**
...
function registerProviders(context) {
    ...
    // Completion provider
    const completionProvider = vscode.languages.registerCompletionItemProvider(PATTERN_SELECTOR, {
    ...
    // Hover provider
    const hoverProvider = vscode.languages.registerHoverProvider(PATTERN_SELECTOR, {
        provideHover(document, position, token) {
            const range = document.getWordRangeAtPosition(position);
            const word = document.getText(range);
            // Show description for pattern types
            if (Object.values(MetricType).includes(word)) {
    ...
    // Diagnostic provider
    const diagnosticCollection = vscode.languages.createDiagnosticCollection('aeims-patterns');
    const diagnosticsProvider = vscode.workspace.onDidChangeTextDocument(event => {
        if (!event.document.fileName.endsWith('.pattern.json'))
            return;
        const diagnostics = [];
        try {
            ...
            if (!validation.valid) {
                validation.errors.forEach(error => {
                    diagnostics.push(new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), error, vscode.DiagnosticSeverity.Error));
                });
            }
            validation.warnings.forEach(warning => {
                diagnostics.push(new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), warning, vscode.DiagnosticSeverity.Warning));
            });
        }
        catch (error) {
            diagnostics.push(new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), `Invalid pattern: ${error.message}`, vscode.DiagnosticSeverity.Error));
        }
        diagnosticCollection.set(event.document.uri, diagnostics);
    });
    context.subscriptions.push(completionProvider, hoverProvider, diagnosticsProvider, diagnosticCollection);
}
/**
 * Get Pattern Type Description
 */
function getPatternTypeDescription(type) {
    switch (type) {
        case 'wave':
 ...
 */
function getPatternDesignerHtml() {
    return `
    <!DOCTYPE html>
    <html lang="en">
    <head>
