...
exports.ProtocolAnalyzer = exports.MessageType = void 0;
 ...
 */
class ProtocolAnalyzer extends events_1.EventEmitter {
    constructor() {
        super();
        this.messages = [];
     ...
     */
    registerDecoder(name, config) {
        this.decoders.set(name, config);
    }
    /**
     * Enable/disable decoders
     */
    setActiveDecoders(decoderNames) {
     ...
     */
    recordMessage(type, data, metadata) {
        const message = {
            type,
            timestamp: Date.now(),
            raw: data,
     ...
     */
    recordResponse(commandId, success, rawData) {
    ...
    detectPatterns(analysis) {
        ...
        for (const [sequence, count] of sequences) {
            if (count >= 3) { // At least 3 occurrences
                analysis.patternDetection.repeatingSequences.push(sequence.split(','));
            }
        }
    ...
    analyzeTiming(analysis) {
        ...
        if (intervals.length > 0) {
            ...
            intervals.forEach(interval => {
                const bucket = Math.min(Math.floor(interval / bucketSize), buckets - 1);
                histogram[bucket]++;
            });
            analysis.timingAnalysis.intervalHistogram = histogram;
            // Detect bursts
            const stdDev = Math.sqrt(intervals.reduce((sum, x) => sum + Math.pow(x - analysis.timingAnalysis.averageInterval, 2), 0) /
                intervals.length);
    ...
    detectAnomalies(analysis) {
        ...
        const stdDev = Math.sqrt(intervals.reduce((sum, x) => sum + Math.pow(x - avgInterval, 2), 0) /
            intervals.length);
     ...
     */
    decodeMessage(data) {
        if (this.activeDecoders.size === 0)
            return undefined;
        for (const [name, config] of this.decoders) {
            if (!this.activeDecoders.has(name))
                continue;
            try {
    ...
    decodeBinary(data, config) {
        ...
        if (config.lengthField) {
            const lengthBytes = data.slice(position + config.lengthField.offset, position + config.lengthField.offset + config.lengthField.size);
        ...
        if (config.checksumField) {
            const checksum = data.slice(position, position + config.checksumField.size);
            const calculated = this.calculateChecksum(data.slice(0, position), config.checksumField.algorithm);
        ...
        let type;
        if (header) {
            if (header[0] === config.commandPrefix)
                type = 'command';
            else if (header[0] === config.responsePrefix)
                type = 'response';
            else if (header[0] === config.eventPrefix)
                type = 'event';
            else if (header[0] === config.errorPrefix)
                type = 'error';
        }
        return {
            type,
     ...
     * Calculate checksum
     */
    calculateChecksum(data, algorithm) {
        switch (algorithm) {
            case 'xor':
    ...
    updateStats(message) {
        ...
        if (message.type === MessageType.RESPONSE && message.metadata?.commandId) {
            const command = this.findMessage(MessageType.COMMAND, msg => msg.metadata?.id === message.metadata.commandId);
            if (command) {
                const latency = message.timestamp - command.timestamp;
                const bucket = Math.min(Math.floor(latency / 100), this.stats.latencyHistogram.length - 1);
     ...
     */
    findMessage(type, predicate) {
        return this.messages.find(msg => msg.type === type && predicate(msg));
    }
}
exports.ProtocolAnalyzer = ProtocolAnalyzer;
//# sourceMappingURL=ProtocolAnalyzer.js.map