...
interface IdentityState {
  ...
  activeChallenge?: IdentityChallenge;
...
export class IdentityService extends EventEmitter {
  private options: Required<IdentityOptions>;
  private states: Map<string, IdentityState> = new Map();
  private challenges: Map<string, IdentityChallenge> = new Map();

  constructor(
    private encryption: DeviceEncryptionService,
    private telemetry: TelemetryManager,
    options: Partial<IdentityOptions> = {}
  ) {
    super();
   ...
   */
  async startVerification(id: string, type: 'password' | 'mfa' | 'device'): Promise<IdentityChallenge> {
   ...
   */
  async refreshToken(id: string, refreshToken: string): Promise<IdentityToken> {
    const state = this.getState(id);
  ...
  private initializeOptions(options: Partial<IdentityOptions>): Required<IdentityOptions> {
    return {
  ...
  private getState(id: string): IdentityState {
  ...
  private async generateChallenge(
    ...
    const buffer = await this.encryption.generateRandomBytes(
      this.options.challengeLength
    );
  ...
  private async generateToken(id: string): Promise<IdentityToken> {
    const now = Date.now();
