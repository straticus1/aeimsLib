...
interface ConfigManagerEvents {
  'configAdded': (config: DeviceConfiguration) => void;
  'configUpdated': (config: DeviceConfiguration) => void;
  'configRemoved': (deviceId: string) => void;
  'schemaAdded': (schema: DeviceConfigSchema) => void;
  'schemaUpdated': (schema: DeviceConfigSchema) => void;
  'schemaRemoved': (schemaId: string) => void;
  'validationError': (deviceId: string, errors: string[]) => void;
}
 ...
 */
export class DeviceConfigManager extends EventEmitter {
  private options: Required<ConfigManagerOptions>;
  private schemas = new Map<string, DeviceConfigSchema>();
  private configs = new Map<string, DeviceConfiguration>();
   ...
   */
  async addSchema(schema: DeviceConfigSchema): Promise<void> {
    // Validate schema
   ...
   */
  async updateSchema(schema: DeviceConfigSchema): Promise<void> {
    // Validate schema
    this.validateSchema(schema);
  ...
  async removeSchema(schemaId: string): Promise<void> {
    ...
    if (configs.length > 0) {
      throw new Error(
        `Cannot remove schema ${schemaId} - in use by ${configs.length} configurations`
      );
    }
    ...
    await this.database.delete(
      `${this.options.storagePrefix}:schema:${schemaId}`
    );
   ...
   */
  async removeConfiguration(deviceId: string): Promise<void> {
    // Check configuration exists
    const config = this.configs.get(deviceId);
    if (!config) {
      return;
    ...
    await this.database.delete(
      `${this.options.storagePrefix}:config:${deviceId}`
    );
  ...
  async validateConfiguration(
    ...
    if (schema.rules) {
      for (const rule of schema.rules) {
        try {
          const valid = await this.evaluateRule(
            rule.condition,
            config.settings
          );
          if (!valid) {
          ...
          this.logger.error(
            `Error evaluating rule for ${config.deviceId}:`,
            error
          );
   ...
   * Initialize options
   */
  private initializeOptions(options: ConfigManagerOptions): Required<ConfigManagerOptions> {
    return {
  ...
  private async loadSchemas(): Promise<void> {
    try {
      ...
      for (const key of keys) {
        ...
        const schema: DeviceConfigSchema = JSON.parse(data);
        this.validateSchema(schema);
        this.schemas.set(schema.id, schema);
      }
  ...
  private async loadConfigurations(): Promise<void> {
    try {
      ...
      for (const key of keys) {
        ...
        const config: DeviceConfiguration = JSON.parse(data);
   ...
   */
  private validateSchema(schema: DeviceConfigSchema): void {
    if (!schema.id) {
    ...
    if (!schema.version) {
      throw new ValidationError('Schema must have a version');
    }
    if (!schema.properties || Object.keys(schema.properties).length === 0) {
      throw new ValidationError('Schema must have properties');
    }

    // Validate property definitions
    for (const [key, prop] of Object.entries(schema.properties)) {
      if (!prop.type) {
        throw new ValidationError(`Property ${key} must have a type`);
      }
        ...
        for (const [subKey, subProp] of Object.entries(prop.properties)) {
          if (!subProp.type) {
            throw new ValidationError(
              `Nested property ${key}.${subKey} must have a type`
            );
          }
        }
      }
      ...
      if (prop.type === 'array' && prop.items) {
        if (!prop.items.type) {
          throw new ValidationError(
            `Array items in ${key} must have a type`
          );
        }
      }
    }
    ...
    if (schema.rules) {
      for (const rule of schema.rules) {
        if (!rule.condition) {
          throw new ValidationError('Rule must have a condition');
        }
        if (!rule.message) {
          throw new ValidationError('Rule must have an error message');
        }
      }
    }
  }
   ...
   */
  private async handleSchemaVersion(
    oldSchema: DeviceConfigSchema,
    newSchema: DeviceConfigSchema
  ): Promise<void> {
    ...
    if (this.options.schemaMigration) {
      for (const config of affected) {
        try {
          // Perform migration
          const migrated = await this.migrateConfiguration(
            config,
            oldSchema,
            newSchema
          );
          ...
          this.logger.error(
            `Failed to migrate configuration ${config.deviceId}:`,
            error
          );
          throw error;
        }
   ...
   */
  private async persistSchema(schema: DeviceConfigSchema): Promise<void> {
   ...
   */
  private async persistConfiguration(config: DeviceConfiguration): Promise<void> {
   ...
   */
  private async pruneConfigurations(): Promise<void> {
    for (const config of this.configs.values()) {
      if (!config.history) continue;
