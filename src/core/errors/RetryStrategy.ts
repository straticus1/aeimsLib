...
export const CommonRetryStrategies = {
  ...
  NONE: {
    maxAttempts: 1,
    backoffType: 'fixed' as RetryBackoffType,
    initialDelay: 0,
    maxDelay: 0
  },
  ...
  PROGRESSIVE: {
    maxAttempts: 5,
    backoffType: 'linear' as RetryBackoffType,
    initialDelay: 1000,
    maxDelay: 10000,
    jitter: true
  },
  ...
  EXPONENTIAL: {
    maxAttempts: 5,
    backoffType: 'exponential' as RetryBackoffType,
    initialDelay: 1000,
    maxDelay: 30000,
    jitter: true
  },
      ...
      if (strategy.timeout) {
        const result = await Promise.race([
          ...
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error('Operation timeout')), strategy.timeout)
          )
        ]);
