...
router.get('/visualization/metrics/realtime', (req, res) => {
  ...
  const { 
    types = Object.values(MetricType),
 ...
 */
export function setupWebSocketEvents(io: any) {
  const metrics = MetricsCollector.getInstance();
  const analytics = AnalyticsCollector.getInstance();
  ...
  io.on('connection', (socket: any) => {
    ...
    socket.on('subscribe_metrics', (category: string) => {
      socket.join(`metrics:${category}`);
    });

    // Unsubscribe from metric category
    socket.on('unsubscribe_metrics', (category: string) => {
