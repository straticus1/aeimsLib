...
export class MetricsCollector extends EventEmitter {
  ...
  private readonly MAX_VALUES = 10000;
  ...
  getMetrics(query: MetricQuery = {}): Record<string, MetricSummary> {
    ...
    for (const [name, config] of this.metrics) {
      ...
      if (query.category && query.category !== config.category) continue;
      if (query.type && query.type !== config.type) continue;
  ...
  private validateMetricValue(value: number, config: MetricConfig): void {
    switch (config.type) {
  ...
  private trimOldValues(name: string, cutoff: number): void {
    const values = this.metricValues.get(name)!;
  ...
  private matchTags(tags: Record<string, string>, queryTags?: Record<string, string>): boolean {
    if (!queryTags) return true;

    return Object.entries(queryTags).every(([key, value]) => 
      tags[key] === value
    );
  }

  private calculateMetricSummary(values: MetricValue[], config: MetricConfig): MetricSummary {
